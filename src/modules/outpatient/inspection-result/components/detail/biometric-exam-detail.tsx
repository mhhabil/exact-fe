...
import BiometricTemplateDisabled from '../../../../outpatient/biometric-examination-results/components/biometric-template-disabled';
...
const BiometricExamDetail = (props: { data: any }) => {
  const { data } = props;
  ...
  const { register, handleSubmit, errors, setValue, getValues, reset, control } = useForm({
    ...
    defaultValues: {
      ...
      od_rp_12_iol_master: data?.Od_Rp_12_Iol_Master ?? '',
      od_rp_12_imersion: data?.Od_Rp_12_Imersion ?? '',
      od_rp_12_contact: data?.Od_Rp_12_Contact ?? '',
      ...
      os_rp_12_iol_master: data?.Os_Rp_12_Iol_Master ?? '',
      os_rp_12_imersion: data?.Os_Rp_12_Imersion ?? '',
      os_rp_12_contact: data?.Os_Rp_12_Contact ?? '',
      ...
      check_od_rp_12: data?.Check_Od_Rp_12 ?? '',
      ...
      check_os_rp_12: data?.Check_Os_Rp_12 ?? '',
      ...
      perawat_pemeriksa_nama: data?.Perawat_Pemeriksa_Nama ?? '',
      perawat_pemeriksa: data?.Perawat_Pemeriksa_Id ?? '',
      ...
      'id-dokter-pemeriksa': data?.Nama_Dokter_Pemeriksa ?? '',
      'ttd-perawat': data && data.TTD_Perawat ? data.TTD_Perawat : '',
      'id-perawat': data && data.ID_Perawat ? data.ID_Perawat : '',
    },
  })
  ...
  return (
    <Form onSubmit={handleSubmit(handleSubmitForm)}>
      ...
      <DicomForm
        dicom={data.dicoms} modality={"BI"}
      />
      <Table borderless style={{ width: '100%' }}>
        <h4 className="mt-2">Form Hasil Pemeriksaan Alat Biometri</h4>
          ...
          <Col>
            {
              biometricList && biometricList.map((val: any, key: number) => {
                const paramLeft = data && data[`Check_Od_${val.id}`] ? data[`Check_Od_${val.id}`] : val.checked;
                const paramRight = data && data[`Check_Os_${val.id}`] ? data[`Check_Os_${val.id}`] : val.checked;
                return (
