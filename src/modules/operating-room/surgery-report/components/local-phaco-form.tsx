import { Button, Col, Input, InputGroup, InputGroupText, Label, ListGroup, ListGroupItem, Nav, NavItem, NavLink, Progress, Row, TabContent, TabPane, Table } from "reactstrap";
...
import localPhaco from "../const/local-phaco/localPhaco";
import localPhaco2 from "../const/local-phaco/localPhaco2";
import localPhacoPostSurgery from "../const/local-phaco/postSurgery";
import localPhacoSurgeryAction from "../const/local-phaco/surgeryAction";
import surgeryDuration from "../const/local-phaco/surgeryDuration";
import usLocalPhaco from "../const/local-phaco/usLocalPhaco";
import { useAppSelector } from "@src/utility/hooks/useAppSelector";

const LocalPhacoForm = (props: { mainRegister: any, mainErrors: any, mainSetValue: any, mainGetValues: any, defaultSelected?: boolean, data: any, processing: boolean, pdfData: any, publicRuntimeConfig: any }) => {
  ...
  const [checkboxLocalPhaco, setCheckboxLocalPhaco] = useState(
    [
      ...localPhaco.map((l: any) => {
        return {
          id: l.id,
          checked: (data && data.form) ? data?.form[`Lokal_Phaco_${l.id}`] : false,
        }
      }),
      ...localPhaco2.map((l: any) => {
        return {
          id: l.id,
          checked: (data && data.form) ? data?.form[`Lokal_Phaco_${l.id}`] : false,
        }
  ...
  const handleSelectAll = (e: any) => {
    ...
    if (checkboxLocalPhaco && Array.isArray(checkboxLocalPhaco)) {
      const newValue = checkboxLocalPhaco.map((c: any) => {
        return {
          id: c.id,
          checked: !!e.target.checked,
        }
      });
      checkboxLocalPhaco.forEach(c => {
        mainSetValue(`lokal-phaco-${c.id}`, (e.target.checked ? '1' : '0'));
      });
      setCheckboxLocalPhaco(newValue);
    }
  }

  useEffect(() => {
    if (data && data.form) {
      setCheckboxLocalPhaco([
        ...localPhaco.map((l: any) => {
          return {
            id: l.id,
            checked: (data && data.form) ? data?.form[`Lokal_Phaco_${l.id}`] : false,
      ...
      checkboxLocalPhaco.forEach(c => {
        mainSetValue(`lokal-phaco-${c.id}`, (data?.form[`Lokal_Phaco_${c.id}`] ? '1' : '0'));
      });
    }
  }, [data]);
  ...
  const handleImage = (image: any) => {
    mainSetValue('lokal-phaco-gambar-mata', image);
  }
  ...
  const isLocalPhacoChecked = (item: any) => {
    if (checkboxLocalPhaco && Array.isArray(checkboxLocalPhaco)) {
      const checkbox = checkboxLocalPhaco.find(i => i.id === item.id);
      if (checkbox) {
  ...
  const handleLocalPhacoCheckboxChanged = (e: any, item: any) => {
    const index = checkboxLocalPhaco.findIndex(i => i.id === item.id);
    if (index >= 0) {
      const newValue = [...checkboxLocalPhaco];
  ...
  return (
    ...
    <TabContent activeTab={activeTab}>
                  ...
                  <tr>
                    ...
                    <td style={{ width: '35%' }}>
                      <Input
                        type="select"
                        className="mt-3"
                        id="cmb-diagnosa-pasca-bedah"
                        name="cmb-diagnosa-pasca-bedah"
                        innerRef={mainRegister()}
                      >
                        <option value="" disabled={true}>--</option>
                        {
                          localPhacoPostSurgery && localPhacoPostSurgery.map((item: any, key: number) => {
                  ...
                  <tr>
                    ...
                    <td style={{ width: '35%' }}>
                      <Input
                        type="select"
                        className="mt-3"
                        id="cmb-tindakan-bedah"
                        name="cmb-tindakan-bedah"
                        innerRef={mainRegister()}
                      >
                        <option value="" disabled={true}>--</option>
                        {
                          localPhacoSurgeryAction && localPhacoSurgeryAction.map((item: any, key: number) => {
                            return <option value={item} key={key}>{ item }</option>;
                          })
                        }
                      </Input>
                  ...
                  <tr>
                    ...
                    <td style={{ width: '35%' }}>
                      <Input
                        ...
                        defaultValue='tanggal-pembedahan'
                        innerRef={mainRegister()}
                      />
                    </td>
                  </tr>
                    ...
                    <Row>
                      <Col>
                        <Input
                          ...
                          type="text"
                          name="obat-obat"
                          defaultValue={(data && data.form && data.form.Obat_Obat && data.form.Obat_Obat !== '') ? data.form.Obat_Obat : 'Lidocaine 2%'}
                          innerRef={mainRegister()}
                        />
                      </Col>
                    </Row>
              ...
              <td>
                <Row className="mt-1">
                  <Col>
                    <Input
                      type="date"
                      defaultValue='tanggal-jaringan-patologi'
      ...
      <TabPane tabId='2'>
        ...
        <Row className="mt-3">
          <Col>
            <Label style={{ fontSize: '12pt' }}>Laporan Pembedahan Phacomulsifikasi</Label>
          </Col>
        </Row>
        <Row className="mt-2">
          <Col>
        ...
        {
          localPhaco && localPhaco.map((item: any, key: number) => (
            <Row className="mb-1" key={key}>
              <Col>
                <Input
                  type="checkbox"
                  name={`lokal-phaco-${key}`}
                  className="me-1"
                  id={`${key}`}
                  // defaultChecked={!!(data && data.form && data.form[`Lokal_Phaco_${key}`])}
                  checked={isLocalPhacoChecked(item)}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    handleLocalPhacoCheckboxChanged(e, item);
                  }}
                  innerRef={mainRegister(`lokal-phaco-${key}`)}
                />
        ...
        <Row className="mb-1 ms-3">
          <Col className="d-flex flex-column">
            <div className="d-flex mb-1">
              <Input
                type="radio"
                name="lokal-phaco-knife"
                className="me-1"
                id="lokal-phaco-knife"
                defaultChecked={!!(data && data.form && data.form.Lokal_Phaco_Knife && data.form.Lokal_Phaco_Knife === '1')}
                value="1"
                onChange={(e) => handleRadioChange(e)}
                innerRef={mainRegister('lokal-phaco-knife')}
              />
              <label>1. slit knife 2.2mm</label>
            </div>
            <div className="d-flex">
              <Input
                ...
                className="me-1"
                id="lokal-phaco-knife"
                defaultChecked={!!(data && data.form && data.form.Lokal_Phaco_Knife && data.form.Lokal_Phaco_Knife === '2')}
                value="2"
                onChange={(e) => handleRadioChange(e)}
                innerRef={mainRegister('lokal-phaco-knife')}
              />
              <label>2. slit knife 2.75mm</label>
            </div>
          </Col>
        </Row>
        {
          localPhaco2 && localPhaco2.map((item: any, key: number) => (
            <Row className="mb-1" key={key}>
              <Col>
                <Input
                  type="checkbox"
                  name={`lokal-phaco-${item.id}`}
                  className="me-1"
                  id={`${item.id}`}
                  checked={isLocalPhacoChecked(item)}
                  onChange={(e) => {
                  ...
                  innerRef={mainRegister(`lokal-phaco-${item.id}`)}
                />
                <label>{item.text}</label>
              </Col>
            </Row>
          ...
          <Col>
            <Table style={{ width: '100%' }} borderless>
              <tr>
                <td style={{ width: '20%' }}>
                  <PhacoImage
                    initialImage={(data && data.form && data.form.Lokal_Phaco_Gambar_Mata && data.form.Lokal_Phaco_Gambar_Mata !== '') ? data.form.Lokal_Phaco_Gambar_Mata : undefined}
                    formName='ok/laporan-pembedahan'
                    component='laporan_pembedahan_lokal_phaco_eye'
                    onSaved={(image: string) => handleImage(image)}
                  />
                  <Input
                    id="lokal-phaco-gambar-mata"
                    type="hidden"
                    name="lokal-phaco-gambar-mata"
                    innerRef={mainRegister({ required: false })}/>
                </td>
                ...
                <td style={{ width: '80%' }}>
                  <Row>
                    ...
                    <Col>
                      ...
                      >
                        {
                          usLocalPhaco && usLocalPhaco.map((item: any, key: number) => {
                            return <option value={item} key={key}>{ item }</option>;
                          })
                        }
                      </Input>
