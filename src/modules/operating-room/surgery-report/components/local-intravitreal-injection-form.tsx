import { Button, Col, Input, InputGroup, InputGroupText, Label, ListGroup, ListGroupItem, Nav, NavItem, NavLink, Progress, Row, TabContent, TabPane, Table, FormGroup } from "reactstrap";
...
import injectionDrip from "../const/local-injection-intravitreal/injectionDrip";
import injectionInjections from "../const/local-injection-intravitreal/injectionInjections";
import injectionIntravitreal from "../const/local-injection-intravitreal/injectionIntravitreal";
import injectionIntravitrealBottom from "../const/local-injection-intravitreal/injectionIntravitrealBottom";
import injectionIntravitrealType from "../const/local-injection-intravitreal/injectionIntravitrealType";
import injectionMeasure from "../const/local-injection-intravitreal/injectionMeasure";
...
const LocalIntravitrealInjectionForm = (props: { mainRegister: any, mainErrors: any, mainSetValue: any, mainGetValues: any, defaultSelected?: boolean, data: any, processing: boolean, pdfData: any, publicRuntimeConfig: any }) => {
  ...
  const [injeksiText, setInjeksiText] = useState((data && data.form && data.form.Lokal_Injeksi_Intravitreal_Injeksi_5) ? !!(data.form.Lokal_Injeksi_Intravitreal_Injeksi_5) : false)
  const { treatment } = useAppSelector(state => state.patient);
  const { doctors } = useAppSelector(state => state.doctor);
  const { nurses } = useAppSelector(state => state.nurse);
  const [inProgress, setInProgress] = useState<any>(undefined);

  const [checkboxInjectionType, setCheckboxInjectionType] = useState(injectionIntravitrealType.map((i: any, key: number) => {
    return {
      id: key + 1,
      name: i,
      checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_Tipe_${key + 1}`] === '1' : false,
    }
  }));

  const [checkboxInjectionIntravitreal, setCheckboxInjectionIntravitreal] = useState(injectionIntravitreal.map((i: any, key: number) => {
    return {
      id: key,
      name: i,
      checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_${key}`] : false,
    }
  }));

  const [checkboxInjectionInjections, setCheckboxInjectionInjections] = useState(injectionInjections.map((i: any, key: number) => {
    return {
      id: key + 1,
      name: i,
      checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_Injeksi_${key + 1}`] === '1' : false,
    }
  }));

  const [checkboxInjectionIntravitrealBottom, setCheckboxInjectionIntravitrealBottom] = useState(injectionIntravitrealBottom.map((i: any, key: number) => {
    return {
      id: i.id,
      name: i.text,
      checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_${i.id}`] : false,
    }
  }));

  const [checkboxInjectionDrip, setCheckboxInjectionDrip] = useState(injectionDrip.map((i: any, key: number) => {
    return {
      id: key + 1,
      name: i,
      checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_Diteteskan_${key + 1}`] : false,
    }
  }));

  const [localInjeksi6, setLocalInjeksi6] = useState(data?.form?.Lokal_Injeksi_Intravitreal_6 && data?.form?.Lokal_Injeksi_Intravitreal_6 === 1 ? '1' : '0');
  const [localInjeksiInjeksi6, setLocalInjeksiInjeksi6] = useState(data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 && data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 === '1' ? '1' : '0');
  const [localInjeksiInjeksi5, setLocalInjeksiInjeksi5] = useState(data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_5 === '1' ? '1' : '0');
  ...
  const handleSelectAll = (e: any) => {
    const newValue = checkboxInjectionType.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxInjectionType.forEach(item => {
      mainSetValue(`lokal-injeksi-intravitreal-tipe-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxInjectionType(newValue);

    const newValue2 = checkboxInjectionIntravitreal.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxInjectionIntravitreal.forEach(item => {
      mainSetValue(`lokal-injeksi-intravitreal-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxInjectionIntravitreal(newValue2);

    const newValue3 = checkboxInjectionInjections.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxInjectionInjections.forEach(item => {
      mainSetValue(`lokal-injeksi-intravitreal-injeksi-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxInjectionInjections(newValue3);

    const newValue4 = checkboxInjectionIntravitrealBottom.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxInjectionIntravitrealBottom.forEach(item => {
      mainSetValue(`lokal-injeksi-intravitreal-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxInjectionIntravitrealBottom(newValue4);

    const newValue5 = checkboxInjectionDrip.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxInjectionDrip.forEach(item => {
      mainSetValue(`lokal-injeksi-intravitreal-diteteskan-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxInjectionDrip(newValue5);

    setLocalInjeksi6(e.target.checked ? '1' : '0');
    setLocalInjeksiInjeksi6(e.target.checked ? '1' : '0');
    setLocalInjeksiInjeksi5(e.target.checked ? '1' : '0');

    mainSetValue('lokal-injeksi-intravitreal-6', e.target.checked ? '1' : '0');
    mainSetValue('lokal-injeksi-intravitreal-injeksi-6', e.target.checked ? '1' : '0');
    mainSetValue('lokal-injeksi-intravitreal-injeksi-5', e.target.checked ? '1' : '0');
  }

  useEffect(() => {
    if (data && data.form) {
      setCheckboxInjectionType(injectionIntravitrealType.map((i: any, key: number) => {
        return {
          id: key + 1,
          name: i,
          checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_Tipe_${key + 1}`] === '1' : false,
      ...
      setCheckboxInjectionIntravitreal(injectionIntravitreal.map((i: any, key: number) => {
        return {
          id: key,
          name: i,
          checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_${key}`] : false,
      ...
      setCheckboxInjectionInjections(injectionInjections.map((i: any, key: number) => {
        return {
          id: key + 1,
          name: i,
          checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_Injeksi_${key + 1}`] === '1' : false,
      ...
      setCheckboxInjectionIntravitrealBottom(injectionIntravitrealBottom.map((i: any, key: number) => {
        return {
          id: i.id,
          name: i.text,
          checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_${i.id}`] : false,
      ...
      setCheckboxInjectionDrip(injectionDrip.map((i: any, key: number) => {
        return {
          id: key + 1,
          name: i,
          checked: (data && data.form) ? data.form[`Lokal_Injeksi_Intravitreal_${key + 1}`] : false,
        }
      }));

      setLocalInjeksi6(data?.form?.Lokal_Injeksi_Intravitreal_6 && data?.form?.Lokal_Injeksi_Intravitreal_6 === 1 ? '1' : '0');
      setLocalInjeksiInjeksi6(data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 && data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 === '1' ? '1' : '0');
      setLocalInjeksiInjeksi5(data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_5 && data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_5 === '1' ? '1' : '0');

      checkboxInjectionType.forEach(item => {
        mainSetValue(`lokal-injeksi-intravitreal-tipe-${item.id}`, (data.form[`Lokal_Injeksi_Intravitreal_Tipe_${item.id}`] === '1') ? '1' : '0');
      });

      checkboxInjectionIntravitreal.forEach(item => {
        mainSetValue(`lokal-injeksi-intravitreal-${item.id}`, data.form[`Lokal_Injeksi_Intravitreal_${item.id}`] ? '1' : '0');
      });

      checkboxInjectionInjections.forEach(item => {
        mainSetValue(`lokal-injeksi-intravitreal-injeksi-${item.id}`, (data.form[`Lokal_Injeksi_Intravitreal_Injeksi_${item.id}`] === '1') ? '1' : '0');
      });
      ...
      checkboxInjectionDrip.forEach(item => {
        mainSetValue(`lokal-injeksi-intravitreal-diteteskan-${item.id}`, data.form[`Lokal_Injeksi_Intravitreal_${item.id}`] ? '1' : '0');
      });

      mainSetValue('lokal-injeksi-intravitreal-6', data?.form?.Lokal_Injeksi_Intravitreal_6 && data?.form?.Lokal_Injeksi_Intravitreal_6 === 1 ? '1' : '0');
      mainSetValue('lokal-injeksi-intravitreal-injeksi-6', data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 && data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_6 === '1' ? '1' : '0');
      mainSetValue('lokal-injeksi-intravitreal-injeksi-5', data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_5 && data?.form?.Lokal_Injeksi_Intravitreal_Injeksi_5 === '1' ? '1' : '0');
    }
  }, [data]);
  ...
  return (
    ...
    <TabContent activeTab={activeTab}>
                    ...
                    <Row>
                      <Col>
                        <Input
                          ...
                          type="text"
                          name="obat-obat"
                          defaultValue={(data && data.form && data.form.Obat_Obat && data.form.Obat_Obat !== '') ? data.form.Obat_Obat : 'pantocain 0,5 %'}
                          innerRef={mainRegister()}
                        />
                      </Col>
                    </Row>
      ...
      <TabPane tabId='2'>
        <Row className="mt-3">
          <Col>
            <Label style={{ fontSize: '12pt' }}>Laporan Pembedahan Injeksi Intravitreal</Label>
          </Col>
        </Row>
        <Row className="mt-2">
          <Col>
            ...
            />
            <label>Pilih Semua Injeksi Intravitreal</label>
          </Col>
        </Row>
        <Row>
          <Col>
        ...
        <Row className="mt-1 mb-4">
          {
            checkboxInjectionType && checkboxInjectionType.map((item: any, key: number) => (
              <Col key={key}>
                <Input
                  id="lokal-injeksi-intravitreal-tipe"
                  type="checkbox"
                  name={`lokal-injeksi-intravitreal-tipe-${item.id}`}
                  className='me-1'
                  // defaultChecked={!!(data && data.form && data.form[`Lokal_Injeksi_Intravitreal_Tipe_${key + 1}`])}
                  checked={item.checked}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    const index = checkboxInjectionType.findIndex(c => c.id === item.id);
                    if (index >= 0) {
                      const newValue = [...checkboxInjectionType];
                      newValue[index].checked = e.target.checked;
                      setCheckboxInjectionType(newValue);
                    }
                  }}
                  innerRef={mainRegister(`lokal-injeksi-intravitreal-tipe-${item.id}`)}
                />
        ...
        {
          checkboxInjectionIntravitreal && checkboxInjectionIntravitreal.map((item: any, key: number) => (
            <Row key={item.id} className='mb-1'>
              <Col>
                <Input
                  id="lokal-injeksi-intravitreal"
                  type="checkbox"
                  name={`lokal-injeksi-intravitreal-${item.id}`}
                  className='me-1'
                  // defaultChecked={!!(data && data.form && data.form[`Lokal_Injeksi_Intravitreal_${item.id}`])}
                  checked={item.checked}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    const index = checkboxInjectionIntravitreal.findIndex(c => c.id === item.id);
                    if (index >= 0) {
                      const newValue = [...checkboxInjectionIntravitreal];
                      newValue[index].checked = e.target.checked;
                      setCheckboxInjectionIntravitreal(newValue);
                    }
                  }}
                  innerRef={mainRegister(`lokal-injeksi-intravitreal-${key}`)}
                />
                <label>{item.name}</label>
              </Col>
            </Row>
        ...
        <Row className="mt-3">
          <Col>
            <label>Dilakukan pengukuran lokasi injeksi dengan menggunakan kaliper</label>
          </Col>
          <Col>
            {
              injectionMeasure && injectionMeasure.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      id="lokal-injeksi-intravitreal-pengukuran"
                      type="radio"
                      name="lokal-injeksi-intravitreal-pengukuran"
                      className="me-1"
                      value={`${key + 1}`}
                      defaultChecked={data && data.form && data.form.Lokal_Injeksi_Intravitreal_Pengukuran === `${key + 1}`}
                      onChange={(e) => handleRadioChange(e)}
                      innerRef={mainRegister("lokal-injeksi-intravitreal-pengukuran")}
                    />
                    <label>{item}</label>
                  </Col>
                </Row>
        ...
        <Row className="mt-1">
          <Col>
            <Input
              id="lokal-injeksi-intravitreal-6"
              type="checkbox"
              name={`lokal-injeksi-intravitreal-6`}
              className='me-1'
              // defaultChecked={!!(data && data.form && data.form.Lokal_Injeksi_Intravitreal_6)}
              checked={localInjeksi6 === '1'}
              onChange={(e) => {
                handleCheckboxChange(e);
                setLocalInjeksi6(e.target.checked ? '1' : '0');
              }}
              innerRef={mainRegister(`lokal-injeksi-intravitreal-6`)}
            />
            <label>Dilakukan displacement konjungtiva dengan forcep</label>
          </Col>
        </Row>
        <Row className="mt-3 mb-3">
          <Col>
            <label>Dilakukan dengan menggunakan syringe 1cc dan needle 30G</label>
          </Col>
          <Col>
            {
              checkboxInjectionInjections && checkboxInjectionInjections.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      type="checkbox"
                      name={`lokal-injeksi-intravitreal-injeksi-${item.id}`}
                      className="me-1"
                      // defaultChecked={!!(data && data.form && data.form[`Lokal_Injeksi_Intravitreal_Injeksi_${item.id}`])}
                      checked={item.checked}
                      onChange={(e) => {
                        handleCheckboxChange(e);
                        const index = checkboxInjectionInjections.findIndex(c => c.id === item.id);
                        if (index >= 0) {
                          const newValue = [...checkboxInjectionInjections];
                          newValue[index].checked = e.target.checked;
                          setCheckboxInjectionInjections(newValue);
                        }
                      }}
                      innerRef={mainRegister(`lokal-injeksi-intravitreal-injeksi-${item.id}`)}
                    />
                    <label>{item.name}</label>
                  </Col>
                </Row>
            ...
            <Row className="mb-1">
              <Col>
                <Input
                  id="lokal-injeksi-intravitreal-injeksi-6"
                  type="checkbox"
                  name="lokal-injeksi-intravitreal-injeksi-6"
                  className="me-1"
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    setLocalInjeksiInjeksi6(e.target.checked ? '1' : '0')
                  }}
                  // defaultChecked={!!(data && data.form && data.form.Lokal_Injeksi_Intravitreal_Injeksi_6)}
                  checked={localInjeksiInjeksi6 === '1'}
                  innerRef={mainRegister('lokal-injeksi-intravitreal-injeksi-6')}
                />
                <label>Injeksi Pagenax 0.05 cc</label>
              </Col>
            </Row>
            <Row className="mb-1">
              <Col>
                <Input
                  id="lokal-injeksi-intravitreal-injeksi-5"
                  type="checkbox"
                  name="lokal-injeksi-intravitreal-injeksi-5"
                  className="me-1"
                  // defaultChecked={!!(data && data.form && data.form.Lokal_Injeksi_Intravitreal_Injeksi_5)}
                  checked={localInjeksiInjeksi5 === '1'}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    setInjeksiText(e.target.checked);
                    setLocalInjeksiInjeksi5(e.target.checked ? '1' : '0');
                  }}
                  innerRef={mainRegister("lokal-injeksi-intravitreal-injeksi-5")}
                />
            ...
            </Row>
            {
              injeksiText && (
                <Row>
                  <Col>
                    <Input
                      id="lokal-injeksi-intravitreal-injeksi-lain-teks"
                      type="text"
                      name="lokal-injeksi-intravitreal-injeksi-lain-teks"
                      className="me-1"
                      innerRef={mainRegister()}
                    />
                  </Col>
        ...
        {
          checkboxInjectionIntravitrealBottom && checkboxInjectionIntravitrealBottom.map((item: any, key: number) => (
            <Row key={key} className='mb-1'>
              <Col>
                <Input
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    const index = checkboxInjectionIntravitrealBottom.findIndex(c => c.id === item.id);
                    if (index >= 0) {
                      const newValue = [...checkboxInjectionIntravitrealBottom];
                      newValue[index].checked = e.target.checked;
                      setCheckboxInjectionIntravitrealBottom(newValue);
                    }
                  }}
                  // defaultChecked={!!(data && data.form && data.form[`Lokal_Injeksi_Intravitreal_${item.id}`])}
                  checked={item.checked}
                  innerRef={mainRegister(`lokal-injeksi-intravitreal-${item.id}`)}
                />
                <label>{item.name}</label>
              </Col>
            </Row>
        ...
        <Row className="mt-3">
          ...
          <Col>
            {
              checkboxInjectionDrip && checkboxInjectionDrip.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      type="checkbox"
                      name={`lokal-injeksi-intravitreal-diteteskan-${item.id}`}
                      className='me-1'
                      onChange={(e) => {
                        handleCheckboxChange(e);
                        const index = checkboxInjectionDrip.findIndex(c => c.id === item.id);
                        if (index >= 0) {
                          const newValue = [...checkboxInjectionDrip];
                          newValue[index].checked = e.target.checked;
                          setCheckboxInjectionDrip(newValue);
                        }
                      }}
                      checked={item.checked}
                      // defaultChecked={!!(data && data.form && data.form[`Lokal_Injeksi_Intravitreal_Diteteskan_${key + 1}`])}
                      innerRef={mainRegister(`lokal-injeksi-intravitreal-diteteskan-${item.id}`)}
                    />
                    <label>{item.name}</label>
                  </Col>
                </Row>
...
export default LocalIntravitrealInjectionForm;
