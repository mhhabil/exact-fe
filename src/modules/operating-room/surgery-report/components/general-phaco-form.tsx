...
import generalPhaco from "../const/general-phaco/generalPhaco";
import generalPhaco2 from "../const/general-phaco/generalPhaco2";
...
const GeneralPhacoForm = (props: { mainRegister: any, mainErrors: any, mainSetValue: any, mainGetValues: any, defaultSelected?: boolean, data: any, processing: boolean, pdfData: any, publicRuntimeConfig: any }) => {
  ...
  const [checkboxGeneralPhaco, setCheckboxGeneralPhaco] = useState([
    ...generalPhaco.map((i: any, key: number) => {
      return {
        id: key,
        name: i,
        checked: (data && data.form) ? data.form[`Umum_Phaco_${key}`] : false,
      }
    }),
    ...generalPhaco2.map((i: any, key: number) => {
      return {
        id: i.id,
        name: i.text,
        checked: (data && data.form) ? data.form[`Umum_Phaco_${i.id}`] : false,
      }
    }),
  ]);

  const handleSelectAll = (e: any) => {
    const newValue = checkboxGeneralPhaco.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxGeneralPhaco.forEach(item => {
      mainSetValue(`umum-phaco-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxGeneralPhaco(newValue);
  }

  useEffect(() => {
    if (data && data.form) {
      setCheckboxGeneralPhaco([
        ...generalPhaco.map((i: any, key: number) => {
          return {
            id: key,
            name: i,
      ...
      checkboxGeneralPhaco.forEach(item => {
        mainSetValue(`umum-phaco-${item.id}`, data.form[`Umum_Phaco_${item.id}`] ? '1' : '0');
      });
    }
  }, [data]);
  ...
  const handleImage = (image: any) => {
    mainSetValue('umum-phaco-gambar-mata', image);
  }
  ...
  return (
    ...
    <TabContent activeTab={activeTab}>
                  ...
                  <tr>
                    ...
                    <td style={{ width: '35%' }}>
                      ...
                      >
                        <option value="" disabled={true}>--</option>
                        {
                          generalPhacoSurgeryAction && generalPhacoSurgeryAction.map((item: any, key: number) => {
                            return <option value={item} key={key}>{ item }</option>;
                          })
                        }
                      </Input>
      ...
      <TabPane tabId='2'>
        ...
        {
          checkboxGeneralPhaco && checkboxGeneralPhaco.map((item: any, key: number) => {
            if (item.id <= 10) {
              return (
                <Row className="mb-1" key={key}>
                  <Col>
                    <Input
                      id="umum-phaco"
                      type="checkbox"
                      name={`umum-phaco-${item.id}`}
                      className="me-1"
                      // defaultChecked={(data && data.form && data.form[`Umum_Phaco_${key}`])}
                      checked={item.checked}
                      onChange={(e) => {
                        handleCheckboxChange(e);
                        const index = checkboxGeneralPhaco.findIndex(c => c.id === item.id);
                        if (index >= 0) {
                          const newValue = [...checkboxGeneralPhaco];
                          newValue[index].checked = e.target.checked;
                          setCheckboxGeneralPhaco(newValue);
                        }
                      }}
                      innerRef={mainRegister(`umum-phaco-${key}`)}
                    />
        ...
        <Row className="mb-1 ms-3">
          <Col className="d-flex flex-column">
            <div className="d-flex mb-1">
              <Input
                type="radio"
                name="umum-phaco-knife"
                className="me-1"
                id="umum-phaco-knife"
                defaultChecked={!!(data && data.form && data.form.Umum_Phaco_Knife && data.form.Umum_Phaco_Knife === '1')}
                value="1"
                onChange={(e) => handleRadioChange(e)}
                innerRef={mainRegister('umum-phaco-knife')}
              />
            ...
            <div className="d-flex">
              <Input
                ...
                className="me-1"
                id="umum-phaco-knife"
                defaultChecked={!!(data && data.form && data.form.Umum_Phaco_Knife && data.form.Umum_Phaco_Knife === '2')}
                value="2"
                onChange={(e) => handleRadioChange(e)}
                innerRef={mainRegister('umum-phaco-knife')}
              />
        ...
        {/*{*/}
        {/*  generalPhaco2 && generalPhaco2.map((item: any, key: number) => (*/}
        {/*    <Row className="mb-1" key={key}>*/}
        {/*      <Col>*/}
        {/*        <Input*/}
        {/*          id="umum-phaco"*/}
        {/*          type="checkbox"*/}
        {/*          name={`umum-phaco-${key + 11}`}*/}
        {/*          className="me-1"*/}
        {/*          defaultChecked={(data && data.form && data.form[`Umum_Phaco_${key + 11}`])}*/}
        {/*          onChange={(e) => handleCheckboxChange(e)}*/}
        {/*          innerRef={mainRegister(`umum-phaco-${key + 11}`)}*/}
        {/*        />*/}
        {/*        <label>{item}</label>*/}
        {/*      </Col>*/}
        {/*    </Row>*/}
          ...
          <Col>
            <Table style={{ width: '100%' }} borderless>
              <tr>
                <td style={{ width: '20%' }}>
                  <PhacoImage
                    initialImage={(data && data.form && data.form.Umum_Phaco_Gambar_Mata && data.form.Umum_Phaco_Gambar_Mata !== '') ? data.form.Umum_Phaco_Gambar_Mata : undefined}
                    formName='ok/laporan-pembedahan'
                    component='laporan_pembedahan_umum_phaco_eye'
                    onSaved={(image: string) => handleImage(image)}
                  />
                  <Input
                    id="umum-phaco-gambar-mata"
                    type="hidden"
                    name="umum-phaco-gambar-mata"
                    innerRef={mainRegister()}/>
                </td>
...
export default GeneralPhacoForm;
