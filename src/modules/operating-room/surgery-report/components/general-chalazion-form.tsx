...
import localChalazionBot from "../const/local-chalazion/localChalazionBot";
import localChalazionDrip from "../const/local-chalazion/localChalazionDrip";
import localChalazionInject from "../const/local-chalazion/localChalazionInject";
import localChalazionKnive from "../const/local-chalazion/localChalazionKnive";
import localChalazionTop from "../const/local-chalazion/localChalazionTop";
import surgeryDuration from "../const/local-phaco/surgeryDuration";
import { useAppSelector } from "@src/utility/hooks/useAppSelector";

const GeneralChalazionForm = (props: { mainRegister: any, mainErrors: any, mainSetValue: any, mainGetValues: any, defaultSelected?: boolean, data: any, processing: boolean, pdfData: any, publicRuntimeConfig: any }) => {
  ...
  const [checkboxLocalChalazionTop, setCheckboxLocalChalazionTop] = useState(localChalazionTop.map((i: any, key: number) => {
    return {
      id: key,
      name: i,
      checked: (data && data.form) ? data.form[`Lokal_Chalazion_${key}`] : false,
    }
  }));

  const [checkboxLocalChalazionBot, setCheckboxLocalChalazionBot] = useState(localChalazionBot.map((i: any) => {
    return {
      id: i.id,
      name: i.text,
      checked: (data && data.form) ? data.form[`Lokal_Chalazion_${i.id}`] : false,
    }
  }));

  const [checkboxLocalChalazionDrip, setCheckboxLocalChalazionDrip] = useState(localChalazionDrip.map((i: any) => {
    return {
      id: i.id,
      name: i.text,
      checked: (data && data.form) ? data.form[`Lokal_Chalazion_Diteteskan_${i.id}`] : false,
    }
  }));

  const [checkboxLocalChalazion3, setCheckboxLocalChalazion3] = useState(data?.form?.Lokal_Chalazion_3 ? '1' : '0');
  const [checkboxLocalChalazion8, setCheckboxLocalChalazion8] = useState(data?.form?.Lokal_Chalazion_8 ? '1' : '0');

  useEffect(() => {
    if (data && data.form) {
      setCheckboxLocalChalazionTop(localChalazionTop.map((i: any, key: number) => {
        return {
          id: key,
          name: i,
          checked: (data && data.form) ? data.form[`Lokal_Chalazion_${key}`] : false,
      ...
      checkboxLocalChalazionTop.forEach(item => {
        mainSetValue(`lokal-chalazion-${item.id}`, data.form[`Lokal_Chalazion_${item.id}`] ? '1' : '0');
      });

      setCheckboxLocalChalazionBot(localChalazionBot.map((i: any) => {
        return {
          id: i.id,
          name: i.text,
          checked: (data && data.form) ? data.form[`Lokal_Chalazion_${i.id}`] : false,
      ...
      checkboxLocalChalazionBot.forEach(item => {
        mainSetValue(`lokal-chalazion-${item.id}`, data.form[`Lokal_Chalazion_${item.id}`] ? '1' : '0');
      });

      setCheckboxLocalChalazionDrip(localChalazionDrip.map((i: any) => {
        return {
          id: i.id,
          name: i.text,
          checked: (data && data.form) ? data.form[`Lokal_Chalazion_Diteteskan_${i.id}`] : false,
      ...
      checkboxLocalChalazionDrip.forEach(item => {
        mainSetValue(`lokal-chalazion-diteteskan-${item.id}`, data.form[`Lokal_Chalazion_Diteteskan_${item.id}`] ? '1' : '0');
      });

      setCheckboxLocalChalazion3(data?.form?.Lokal_Chalazion_3 ? '1' : '0');
      setCheckboxLocalChalazion8(data?.form?.Lokal_Chalazion_8 ? '1' : '0');

      mainSetValue(`lokal-chalazion-3`, data.form[`Lokal_Chalazion_3`] ? '1' : '0');
      mainSetValue(`lokal-chalazion-8`, data.form[`Lokal_Chalazion_8`] ? '1' : '0');
    }
  }, [data]);

  const handleSelectAll = (e: any) => {
    const newValue = checkboxLocalChalazionTop.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxLocalChalazionTop.forEach(item => {
      mainSetValue(`lokal-chalazion-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxLocalChalazionTop(newValue);

    const newValue2 = checkboxLocalChalazionBot.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    ...
    setCheckboxLocalChalazionBot(newValue2);

    const newValue3 = checkboxLocalChalazionDrip.map(c => {
      const a = {...c};
      a.checked = e.target.checked;
      return a;
    });
    checkboxLocalChalazionDrip.forEach(item => {
      mainSetValue(`lokal-chalazion-diteteskan-${item.id}`, e.target.checked ? '1' : '0');
    });
    setCheckboxLocalChalazionDrip(newValue3);

    setCheckboxLocalChalazion3(e.target.checked ? '1' : '0');
    mainSetValue(`lokal-chalazion-3`, e.target.checked ? '1' : '0');
    setCheckboxLocalChalazion8(e.target.checked ? '1' : '0');
    mainSetValue(`lokal-chalazion-8`, e.target.checked ? '1' : '0');
  }
  ...
  const handleCheckboxDrip = (val: any) => {
    mainSetValue(`${val.target.name}`, (val.target.checked) ? `${val.target.id}` : '')
  }
  ...
  const handleImagePra = (image: any) => {
    mainSetValue('lokal-chalazion-gambar-pra', image);
  }

  const handleImagePasca = (image: any) => {
    mainSetValue('lokal-chalazion-gambar-pasca', image);
  }
  ...
  return (
    <><Nav tabs>
      <NavItem>
        <NavLink id="diagnosa" className={(activeTab && activeTab === '1') ? 'active' : ''} onClick={() => toggle('1')}>
          Diagnosa
        </NavLink>
      </NavItem>
      <NavItem>
        <NavLink id="laporan-pembedahan" className={(activeTab && activeTab === '2') ? 'active' : ''} onClick={() => toggle('2')}>
          Laporan Pembedahan
        </NavLink>
      </NavItem>
    </Nav>
    <TabContent activeTab={activeTab}>
      <TabPane tabId='1'>
        <Row>
          ...
          <Table className="w-full mt-3" borderless>
            <tr>
              <td style={{ width: '15%' }}>
                <Label id="dokter-operator" className="mt-1" style={{ fontSize: '10pt' }}>Dokter Operator</Label>
              </td>
              <td>
                <Input
                  className="mt-1"
            ...
            <tr>
              <td style={{ width: '15%' }}>
                <Label id="perawat-asisten-operator" className="mt-3" style={{ fontSize: '10pt' }}>Perawat Asisten Operator</Label>
              </td>
              <td>
                <Input
                  className="mt-3"
            ...
            <tr>
              ...
              <td>
                <Row className="mt-1">
                  <Col>
                    <Input
                      id="jumlah-pendarahan"
                      type="number"
                      innerRef={mainRegister()}
                      invalid={mainErrors['jumlah-pendarahan'] && true}
                      name='jumlah-pendarahan'
                    />
                  </Col>
                </Row>
              </td>
      ...
      <TabPane tabId='2'>
        <Row className="mt-3">
          <Col>
            <Label style={{ fontSize: '12pt' }}>Laporan Pembedahan Chalazion</Label>
          </Col>
        </Row>
        <Row className="mt-2">
          <Col>
            ...
            />
            <label>Pilih Semua Chalazion</label>
          </Col>
        </Row>
        <Row>
          <Col>
        ...
        {
          checkboxLocalChalazionTop && checkboxLocalChalazionTop.map((item: any, key: number) => (
            <Row key={key} className="mt-1">
              <Col>
                <Input
                  type="checkbox"
                  name={`lokal-chalazion-${item.id}`}
                  className='me-1'
                  // defaultChecked={(data && data.form && data.form[`Lokal_Chalazion_${key}`] === 1)}
                  checked={item.checked}
                  innerRef={mainRegister(`lokal-chalazion-${item.id}`)}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    const index = checkboxLocalChalazionTop.findIndex(c => c.id === item.id);
                    if (index >= 0) {
                      const newValue = [...checkboxLocalChalazionTop];
        ...
        <Row className="mt-3">
          ...
          <Col>
            {
              localChalazionInject && localChalazionInject.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      id="lokal-chalazion-injeksi"
                      type="radio"
                      name="lokal-chalazion-injeksi"
                      className="me-1"
                      value={`${key + 1}`}
                      defaultChecked={(data && data.form && data.form.Lokal_Chalazion_Injeksi === `${ key + 1 }`)}
                      innerRef={mainRegister("lokal-chalazion-injeksi")}
                      onChange={(e) => handleRadioChange(e)}
                    />
                    <label>{item}</label>
                  </Col>
        ...
        <Row className="mt-1 mb-1">
          <Col>
            <Input
              id="lokal-chalazion-3"
              type="checkbox"
              name={`lokal-chalazion-3`}
              className='me-1'
              // defaultChecked={(data && data.form && data.form.Lokal_Chalazion_3 === 1)}
              checked={checkboxLocalChalazion3 === '1'}
              innerRef={mainRegister(`lokal-chalazion-3`)}
              onChange={(e) => {
                handleCheckboxChange(e);
                setCheckboxLocalChalazion3(e.target.checked ? '1' : '0');
              }}
            />
            <label>Dilakukan pemasangan chalazion forcep</label>
          </Col>
        </Row>
        <Row className="mt-2 mb-2">
          <Col>
            <label>Dilakukan chalazion dengan menggunakan knive MVR 20%</label>
          </Col>
          <Col>
            {
              localChalazionKnive && localChalazionKnive.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      id="lokal-chalazion-bagian"
                      type="radio"
                      name="lokal-chalazion-bagian"
                      className="me-1"
                      value={`${key + 1}`}
                      defaultChecked={data && data.form && data.form.Lokal_Chalazion_Bagian === `${key + 1}`}
                      innerRef={mainRegister('lokal-chalazion-bagian')}
                      onChange={(e) => handleRadioChange(e)}
                    />
                    <label>{item}</label>
                  </Col>
        ...
        {
          checkboxLocalChalazionBot && checkboxLocalChalazionBot.map((item: any, key: number) => (
            <Row key={key} className='mb-1'>
              <Col>
                <Input
                  type="checkbox"
                  name={`lokal-chalazion-${item.id}`}
                  className='me-1'
                  // defaultChecked={(data && data.form && data.form[`Lokal_Chalazion_${item.id}`] === 1)}
                  checked={item.checked}
                  innerRef={mainRegister(`lokal-chalazion-${item.id}`)}
                  onChange={(e) => {
                    handleCheckboxChange(e);
                    const index = checkboxLocalChalazionBot.findIndex(c => c.id === item.id);
                    if (index >= 0) {
                      const newValue = [...checkboxLocalChalazionBot];
                      newValue[index].checked = e.target.checked;
                      setCheckboxLocalChalazionBot(newValue);
                    }
                  }}
                />
                <label>{item.name}</label>
        ...
        <Row className="mt-3">
          ...
          <Col>
            {
              checkboxLocalChalazionDrip && checkboxLocalChalazionDrip.map((item: any, key: number) => (
                <Row key={key} className='mb-1'>
                  <Col>
                    <Input
                      type="checkbox"
                      id={`${item.id}`}
                      name={`lokal-chalazion-diteteskan-${item.id}`}
                      className='me-1'
                      // defaultChecked={!!(data && data.form && data.form[`Lokal_Chalazion_Diteteskan_${item.id}`])}
                      checked={item.checked}
                      innerRef={mainRegister(`lokal-chalazion-diteteskan-${item.id}`)}
                      onChange={(e) => {
                        handleCheckboxChange(e);
                        const index = checkboxLocalChalazionDrip.findIndex(c => c.id === item.id);
                        if (index >= 0) {
                          const newValue = [...checkboxLocalChalazionDrip];
                          newValue[index].checked = e.target.checked;
                          setCheckboxLocalChalazionDrip(newValue);
                        }
                      }}
                    />
                    <label>{item.name}</label>
        ...
        <Row className='mb-1 mt-2'>
          <Col>
            <Input
              id="lokal-chalazion-8"
              type="checkbox"
              name={`lokal-chalazion-8`}
              className='me-1'
              // defaultChecked={!!(data && data.form && data.form.Lokal_Chalazion_8)}
              checked={checkboxLocalChalazion8 === '1'}
              innerRef={mainRegister(`lokal-chalazion-8`)}
              onChange={(e) => {
                handleCheckboxChange(e);
                setCheckboxLocalChalazion8(e.target.checked ? '1' : '0');
              }}
        ...
        <Table style={{ width: '100%' }} className='mt-3 mb-3' borderless>
          <tr>
            <td style={{ width: '50%', textAlign: 'center' }}>
              <EyeImage
                initialImage={(data && data.form && data.form.Lokal_Chalazion_Gambar_Pra && data.form.Lokal_Chalazion_Gambar_Pra !== '') ? data.form.Lokal_Chalazion_Gambar_Pra : undefined}
                formName='ok/laporan-pembedahan'
                component='laporan_pembedahan_umum_chalazion_pra'
                onSaved={(image: string) => handleImagePra(image)}
              />
              <Label>Gambar Pra</Label>
              <Input
                id="lokal-chalazion-gambar-pra"
                type="hidden"
                name="lokal-chalazion-gambar-pra"
                innerRef={mainRegister('lokal-chalazion-gambar-pra')}/>
            </td>
            <td style={{ width: '50%', textAlign: 'center' }}>
              <EyeImage
                initialImage={(data && data.form && data.form.Lokal_Chalazion_Gambar_Pasca && data.form.Lokal_Chalazion_Gambar_Pasca !== '') ? data.form.Lokal_Chalazion_Gambar_Pasca : undefined}
                formName='ok/laporan-pembedahan'
                component='laporan_pembedahan_umum_chalazion_pasca'
                onSaved={(image: string) => handleImagePasca(image)}
              />
              <Label>Gambar Pasca</Label>
              <Input
                id="lokal-chalazion-gambar-pasca"
                type="hidden"
                name="lokal-chalazion-gambar-pasca"
                innerRef={mainRegister('lokal-chalazion-gambar-pasca')}/>
            </td>
          </tr>
        </Table>
      </TabPane>
...
export default GeneralChalazionForm;
