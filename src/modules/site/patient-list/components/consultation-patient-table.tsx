...
const ConsultationPatientTable = () => {

  const { filter, consultPatients } = useAppSelector(state => state.patient);
  ...
  useEffect(() => {
    if (userData && userData.isDokter && !userData.isRM) {
      const params = PatientFilterRequest.createFromJson({ ...filter, isDokter: 1, isRM: 0, isOther: 0 });
      dispatch(fetchConsultationPatients(params))
    } else if (userData && userData.isRM) {
      dispatch(fetchConsultationPatients(filter))
    } else {
      dispatch(fetchConsultationPatients(filter))
    }
  }, [filter, userData]);
    ...
    <>
      ...
      <Row>
        <Table responsive>
          ...
          <tbody>
            {
              consultPatients && consultPatients.records.map((patient, key) => {
                return (
      ...
      <Row>
        <AppPagination
          currentPage={consultPatients.currentPage}
          totalPages={consultPatients.totalPage}
          itemsPerPage={filter.limit}
          totalItems={consultPatients.total}
          onChangePage={
            (page: number) => {
...
export default ConsultationPatientTable;
