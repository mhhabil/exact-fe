...
import { GrantAccessMRRequest, IGrantAccessMRRequest, ListMRFilterRequest } from "../../requests";
import { fetchAllOfficers, fetchApprovalHistories, fetchMrs, handleFilterListMr, handleMrList } from "../../stores/request-mr.store";
import AppPagination from "@src/shared/pagination/components";
import GrantAccessLimit from "./grant-access-limit";
...
const GrantAccessForm = (props: { onSuccessSubmit: any }) => {
  const { onSuccessSubmit } = props;
  const dispatch = useAppDispatch()
  const { mrList, mrs, filterListMr, filterApproval } = useAppSelector(state => state.requestMr);
  const { companyCode } = useAppSelector(state => state.selectCompany);
  const { officers } = useAppSelector(state => state.requestMr);
  ...
  const handleSubmitForm = (value: IGrantAccessMRRequest) => {
    setProcessing(true)
    const params = GrantAccessMRRequest.createFromJson(value);
    RequestMRService().grantAccess(params)
      .then(() => {
        onSuccessSubmit();
        setProcessing(false);
  ...
  const handleChangeUser = (e: any) => {
    const selected = officers && officers.find((item: any) => item.ID_Karyawan === e.target.value);
    if (selected) {
      setValue('requested_by_name', selected.Nama);
    }
  }
    ...
    <Fragment>
      <Form onSubmit={handleSubmit(handleSubmitForm)}>
        ...
        <FormGroup className="form-group my-1 align-items-center" row>
          <Col md='3'>
            <Label>User Yang Diberi Akses*</Label>
          </Col>
          <Col md='7'>
            <Input
              type="select"
