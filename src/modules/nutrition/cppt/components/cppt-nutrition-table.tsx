...
import { fetchCpptNutrition, fetchCpptNutritionDayPdf, fetchCpptNutritionPdf, handlePdf, handlePdfAll } from "../stores/cppt-nutrition.store";
import { AppRequest } from "@src/shared/request";
import { CpptModel } from "@src/modules/ro/cppt/models/cppt-ro.model";
import CpptNutritionDetail from "./cppt-nutrition-detail";
import CpptNutritionForm from "./cppt-nutrition-form";
...
import CpptInpatientDetail from "@src/modules/inpatient/cppt/components/cppt-inpatient-detail";
import { DateTimeConverter } from "@src/shared/datetime-converter";
import CpptEmergencyRoomDetail from "@src/modules/emergency-room/cppt/components/cppt-emergency-room-detail";

const CpptNutritionTable = (props: { data: CpptModel }) => {
  const { data } = props;

  const { treatment } = useAppSelector(state => state.patient);
  const { pdf, pdfAll } = useAppSelector(state => state.cpptNutrition);
  ...
  const handleDeleteRow = (row: any) => {
    ...
    CpptNutritionService().delete(params).then(() => {
      const appRequest = AppRequest.createFromStore(treatment);
      dispatch(fetchCpptNutrition({ ...appRequest, sort }))
  ...
  const handleSort = (sortDir: string) => {
    ...
    const appRequest = AppRequest.createFromStore(treatment);
    dispatch(fetchCpptNutrition({ ...appRequest, sort: sortDir }))
    setSort(sortDir);
  }
  ...
  return (
    <Fragment>
      <Row id='form'>
        <Col md={12}>
          {
            showCreateForm && doctorPreliminaryStudy && (
              <CpptNutritionForm
                onSuccessSubmit={() => {
                  setShowCreateForm(false);
        ...
        <ModalBody>
          {
            doctorPreliminaryStudy && (
              <CpptNutritionDetail cpptGizi={validateRow} validate onSuccessValidate={() => {
...
export default CpptNutritionTable;
