...
export default async function handler(
  ...
  if (req.method === 'POST' && token) {
    const params = UpdateDoctorPreliminaryStudyRequest.createFromJson(req.body).normalize();
    const response = await axios.post(
      `${publicRuntimeConfig.env?.apiv2Url}/rawat-jalan/pengkajian-awal-process`,
      params,
      {...{
        headers: {
          'x-token': token,
