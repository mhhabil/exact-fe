...
export default async function handler(
  ...
  if (req.method === 'POST' && token) {
    const params = UpdateHospitalizationLetterRequest.createFromJson(req.body).normalize();
    const response = await axios.post(
      `${publicRuntimeConfig.env?.apiv2Url}/rawat-inap/surat-perintah-rawat-inap-process`,
      params,
      {...{
        headers: {
          'x-token': token,
