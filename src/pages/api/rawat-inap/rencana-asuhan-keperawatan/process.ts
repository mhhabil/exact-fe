...
export default async function handler(
  ...
  if (req.method === 'POST' && token) {
    const params = UpdateNursingCarePlanRequest.createFromJson(req.body).normalize();
    const response = await axios.post(
      `${publicRuntimeConfig.env?.apiv2Url}/rawat-inap/rencana-asuhan-keperawatan-process`,
      params,
      {...{
        headers: {
          'x-token': token,
