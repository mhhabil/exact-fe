...
export default async function handler(
  ...
  if (req.method === 'POST' && token) {
    const params = UpdateInitialNursingAssessmentChildrenRequest.createFromJson(req.body);
    const response = await axios.post(
      `${publicRuntimeConfig.env?.apiv2Url}/rawat-inap/pengkajian-awal-keperawatan-anak-process`,
      params,
      {...{
        headers: {
          'x-token': token,
