...
import { Circle, Ellipse, Layer, Line, Stage, Text } from 'react-konva';
...
import { RetinaImageService } from '../services';
import StarImage from '../../eye-image/components/star-image'
import { UploadToCloudService } from '@src/shared/upload-cloud-storage/services';
import { useAppSelector } from '@hooks/useAppSelector';

const RetinaImageModal = (props: { isOpen: boolean, onClose: any, title: string, onImageSubmit: any, image: any, type: string, formName: string, component: string }) => {

  const { image, isOpen, title, onClose, onImageSubmit, type, formName, component } = props;
  ...
  const handleReset = () => {
    ...
    if (type === 'right') {
      setPic('/assets/default/retina-right.jpg');
    }
    if (type === 'left') {
      setPic('/assets/default/retina-left.jpg');
    }
  }
  ...
  //   if (!treatment) return;
  //   if (ref && ref.current) {
  //     RetinaImageService().uploadImage({
  ...
  //       component_id: component,
  //       form_name: formName,
  //       image: ref.current.toDataURL(),
  //     }).then(response => {
  //       const { data } = response.data;
  //       onImageSubmit(data.url);
  //       onClose();
  //     });
  //   }
  //   setWidth(0);
  ...
  const handleUndo = () => {
    if (step && step !== [] && step[step.length - 1] === 1) {
      ...
      setStep(cutStep);
    } else if (step && step !== [] && step[step.length - 1] === 2) {
      ...
      const cutStep = step.slice(0, step.length - 1);
      setStep(cutStep);
    } else if (step && step !== [] && step[step.length - 1] === 3) {
      ...
      const cutStep = step.slice(0, step.length - 1);
      setStep(cutStep);
    } else if (step && step !== [] && step[step.length - 1] === 4) {
      ...
      const cutStep = step.slice(0, step.length - 1);
      setStep(cutStep);
    } else if (step && step !== [] && step[step.length - 1] === 5) {
      ...
      const cutStep = step.slice(0, step.length - 1);
      setStep(cutStep);
    } else if (step && step !== [] && step[step.length - 1] === 6) {
            ...
            <div className="signature-canvas">
              <Stage
                ...
                <Layer>
                  ...
                  {
                    (drawer === '1' || lines !== []) && lines.map((line: any, i: number) => (
                      <Line
                        key={i}
                        points={line.points}
                        stroke={line.color}
                  ...
                  {
                    (drawer === '2' || ellipse !== []) && ellipse.map((shape: any, i: number) => (
                      <Ellipse
                        key={i}
                        x={shape.pointX}
                        y={shape.pointY}
                  ...
                  {
                    (drawer === '3' || text !== []) && text.map((txt: any, i: number) => (
                      <Text
                        key={i}
                        text={txt.value}
                        fontSize={20}
                  ...
                  {
                    (drawer === '4' || stars !== []) && stars.map((star: any, i: number) => (
                      <StarImage
                        key={i}
                  ...
                  {
                    (drawer === '5' || dots !== []) && dots.map((dot: any, i: number) => (
                      <Circle
                        key={i}
                        x={dot.dotX}
                        y={dot.dotY}
                  ...
                  {
                    (drawer === '6' || checklist !== []) && checklist.map((check: any, i: number) => (
                      <ChecklistImage
                        key={i}
...
export default RetinaImageModal;
